...
function setup(t) {
  ...
  db.prepare(`INSERT INTO repository_collaborators
  ...
  db.prepare(`INSERT INTO repository_webhooks
  ...
  return { config, db, adminId, memberId, primaryOrgId, otherOrgId, repoId, otherRepoId, emailId, deliveryId };
}
...
async function listen(t, context) {
  const api = createInstanceAdminApiHandlerSafe(context);
...
test('instance admin authority fails closed for tenant members', async (t) => {
...
test('overview and search return operational metadata without secrets', async (t) => {
  const context = setup(t);
  const origin = await listen(t, context);
...
test('tenant detail is scoped and recursively redacts audit secrets', async (t) => {
  const context = setup(t);
  const origin = await listen(t, context);
...
test('support notes require tenant confirmation and create an explicit audit event', async (t) => {
  const context = setup(t);
  const origin = await listen(t, context);
  const adminCookie = cookie(context.db, context.adminId);
  ...
  const auditRow = context.db.prepare(`
    ...
    WHERE target_id = ? AND action = 'instance_support.note_added' LIMIT 1
  `).get(context.primaryOrgId);
  assert.equal(auditRow.action, 'instance_support.note_added');
});

test('failed delivery retries require confirmation, CSRF protection and audit', async (t) => {
  const context = setup(t);
  const origin = await listen(t, context);
  const adminCookie = cookie(context.db, context.adminId);
  ...
  const actions = context.db.prepare(`SELECT action FROM audit_logs WHERE action LIKE 'instance_support.%'`).all().map((row) => row.action);
  assert.ok(actions.includes('instance_support.email_retried'));
  assert.ok(actions.includes('instance_support.webhook_retried'));
});

test('user detail returns counts without credential material', async (t) => {
  const context = setup(t);
  context.db.prepare(`INSERT INTO personal_access_tokens
