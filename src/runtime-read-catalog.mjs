...
function validateSpec(spec) {
  ...
  if (!/^[a-z][a-z0-9_.-]{2,100}$/.test(spec.id)) throw new Error(`Runtime read ID is invalid: ${spec.id}`);
  ...
  if (!/^SELECT\b/i.test(normalizedSql(spec.sqliteSql)) || !/^SELECT\b/i.test(normalizedSql(spec.sampleSql))) {
    throw new Error(`Runtime read catalog accepts SELECT statements only: ${spec.id}`);
  }
  return {
    ...spec,
