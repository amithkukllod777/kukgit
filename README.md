...
- One npm dependency (`pg`), lazily loaded and disabled by default — the instance runs with `node_modules` removed
...
A workflow reaches that storage through two built-in steps — `kukgit/cache@v1` and `kukgit/upload-artifact@v1` — implemented by the agent itself, so nothing is fetched and there is no third-party code on the runner to review. The cache step restores where it appears and **saves after the job**, because the content a cache holds does not exist when the step runs; it is skipped on an exact hit and skipped when the job failed, since a cache written from a broken build is one every later build restores. A cache service that is unreachable never fails a build.

Git LFS objects can live in an **S3-compatible bucket** instead of the instance volume, so one disk stops being the ceiling on what a customer can store. Filesystem remains the default: switching an instance whose objects are already on a volume would make every existing object unreadable, so moving them is a migration rather than a configuration change. SigV4 is written out rather than taken from an SDK — an SDK would be the largest thing in the product for one signing algorithm — and the published AWS test vector is in the suite, because signing is the one part that cannot be checked by round-tripping against our own code. A backup taken against a bucket **verifies** every object by reading it back and re-hashing it but does not copy it into the archive, and the manifest says so: an operator who believes a 40 GB archive holds their 4 TB of objects has a recovery plan that fails the first time it is needed. An instance that already has objects on a volume moves them with `npm run lfs:storage` — plan, copy, verify, then reclaim, as four separate commands because only the last one destroys anything and a migration that deletes its own source has no rollback. Read [Object Storage](docs/OBJECT_STORAGE.md).

A push that introduces a credential can be **refused outright**, before the objects are accepted — because once a credential is in history it is compromised, and rewriting does not un-send bytes that already reached a server. It is off until an administrator enables it per repository, and a bypass names the one fingerprint it covers, expires in thirty minutes, and is recorded with a written reason: a bypass that is not recorded is a control that is not enforced. It **fails closed**, because a control that opens on its own failure is not one. Read [Push Protection](docs/PUSH_PROTECTION.md).

An organization can be deleted, and the deletion **proved**. The list of tables to clear is derived from the schema rather than written down — a hand-maintained list is right the day it is written and wrong by the next migration, and the failure is silent. A table nobody has classified fails the deletion outright, because that is evidence nobody looked rather than evidence of a clean result. The derivation found two real gaps the first time it ran: encrypted secrets were invisible to it, and repositories were linked through a nullable column that is null for every live repository. Read [Tenant Deletion](docs/TENANT_DELETION.md).
...
On `SIGTERM` the process **drains** rather than stopping: readiness starts failing while it is still serving, so the load balancer removes it from rotation *before* the socket closes — skipping that one step is what turns an invisible deploy into a burst of 502s. In-flight requests then finish, with Git transfers on their own five-minute budget because a clone legitimately takes minutes and killing it wastes every byte already sent. `npm run drill` rehearses the whole sequence against a disposable instance. Read [Operations Boundary](docs/OPERATIONS_BOUNDARY.md).

Schema migrations run under SQLite's writer lock, so two instances starting in the same instant no longer race — the second waits and then finds everything applied. Getting there needed three things that were each wrong on their own: `db.transaction` had to become nesting-aware, because twelve modules already open a transaction inside their own migration; `busy_timeout` had to move to the front of its `PRAGMA` block, because everything before it was running with a timeout of zero; and WAL mode had to be established by reading before setting, because changing journal mode takes an exclusive lock and does not wait for it the way an ordinary write does. Read [Operations Boundary](docs/OPERATIONS_BOUNDARY.md).

Background work — email, webhooks, alerts, schedules, retention — runs behind a **named lease**, so two instances against the same volume own one job each instead of both doing all of them. One SQL statement decides ownership, because two instances that both *read* "expired" would both conclude they had won and send every queued message twice. Acquiring and renewing are the same call, so a tick is the heartbeat and there is no separate path that could stop while the work carries on; a lease that cannot be read is not permission to run. Rows a worker claimed and never finished are reclaimed by **age**, never wholesale — resetting every `processing` row, which the webhook migration used to do at startup, resurrects work another instance is performing right now. Read [Operations Boundary](docs/OPERATIONS_BOUNDARY.md).

Not every run starts from a push. A `schedule:` block is honoured **only on the default branch** — read from any ref, it would let anyone who can push a branch install recurring work on the instance that outlives their branch. Cron is evaluated in UTC, because a schedule read in a local zone runs twice on one day each year and not at all on another with nothing in the file to explain it, and missed ticks are never backfilled: an instance down overnight owes one run per schedule, not one per minute it was asleep. Every instance runs the sweep and a single SQL statement decides who holds the lease, so three instances fire each schedule once. Manual runs need repository **write** — starting a workflow runs the repository's own code with its own secrets — and their inputs must be declared in the file, because an undeclared input would be an environment variable the author never wrote, chosen by whoever pressed the button. Read [Workflow Triggers](docs/WORKFLOW_TRIGGERS.md).
